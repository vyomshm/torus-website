...
export default {
  async addTKey({ dispatch, state }, { calledFromEmbed, shareStores }) {
    try {
      ...
      let allKeys = await thresholdKeyController.login(finalKey.privateKey, undefined, shareStores)
  ...
  async calculatePostboxKey({ state, commit, dispatch }, payload) {
    try {
      ...
      let postboxVerifier = currentVerifierConfig.linkedVerifier
      if (!postboxVerifier) {
        postboxVerifier = currentVerifierConfig.verifier
      }
      const postboxKey = await dispatch('getTorusKey', {
        verifier: postboxVerifier,
